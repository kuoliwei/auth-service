...
> 錯誤碼查表、Config 外部化），以及後續整合稽核 T6/T12/T22 的結果（改經 api-gateway `/internal/users`
> 呼叫 user-service、回應格式改為 `{error,message}` / 直接物件（無 `{status,message,data}` 包裝）、
> `EMAIL_ALREADY_EXISTS` 改回 409）。最後對照時間：與 `src/` 現況一致。

## Purpose

auth-service 是 Persona Nexus 平台唯一的**認證服務**，只做兩件事：**使用者註冊**與**登入**，
並在登入成功時**簽發 JWT**。本服務**沒有自己的資料庫**，使用者資料的存取一律透過 HTTP 委派，
經 api-gateway 的 `/internal/users` 路由轉發給 user-service（不直連 user-service）。
...
api-gateway :8000  ── proxy ──▶  auth-service :3000
   ...
   services/authService.js        … 業務邏輯：查重、bcrypt、呼叫 jwtHelper 簽 JWT
                                       ▼
   repositories/userRepository.js … fetch → api-gateway {GATEWAY_URL}/internal/users
                                       ▼
                                 api-gateway :8000 /internal/users → user-service :4000  ← 真正存使用者的地方
...
- **THEN** 系統以 bcrypt 雜湊密碼、產生 `usr_<timestamp>` id、經 api-gateway `/internal/users` 委派 user-service 儲存，回傳 HTTP 201 與直接物件 `{ id, email }`（無 `{status,message,data}` 包裝）

#### Scenario: email 已存在
- **WHEN** 收到註冊請求，但 user-service 查得該 email 已存在
- **THEN** service 拋出 `EMAIL_ALREADY_EXISTS`，controller 回傳 HTTP 409 與 `{ error:"EMAIL_ALREADY_EXISTS", message:"該電子郵件已被註冊，請更換帳號。" }`

#### Scenario: 註冊時發生非預期錯誤
- **WHEN** 註冊過程中 user-service 呼叫失敗或發生未預期例外
- **THEN** service 將其包裝為 `UNKNOWN_SERVER_ERROR`（未命中 `ERROR_MAP`），controller 回傳 HTTP 500 與 `{ error:"INTERNAL_SERVER_ERROR", message:"伺服器內部發生錯誤，請稍後再試。" }`
...
- **THEN** 系統以 `generateToken(userId)` 簽發 JWT，回傳 HTTP 200 與直接物件 `{ id, email, token }`（無 `{status,message,data}` 包裝）

#### Scenario: email 不存在
- **WHEN** 收到登入請求，但 user-service 查無此 email
- **THEN** service 拋出 `UNKNOWN_USER`，controller 回傳 HTTP 400 與 `{ error:"UNKNOWN_USER", message:"Email或密碼錯誤，請輸入正確的Email或密碼。" }`

#### Scenario: 密碼錯誤
- **WHEN** email 存在但 `bcrypt.compare()` 回傳 false
- **THEN** service 拋出 `EMAIL_OR_PASSWORD_NOTMATCH`，controller 回傳 HTTP 400 與 `{ error:"EMAIL_OR_PASSWORD_NOTMATCH", message:"Email或密碼錯誤，請輸入正確的Email或密碼。" }`
...
api-gateway 的 `/internal/users` 路由（`GATEWAY_URL`，預設 `http://localhost:8000`），由 gateway 轉發給
user-service；本服務 MUST NOT 直連 user-service。

#### Scenario: 查詢 email
- **WHEN** service 需確認 email 是否存在
- **THEN** `userRepository.findByEmail(email)` 發 `GET {GATEWAY_URL}/internal/users?email=<email>`；200 回傳使用者、404 回傳 null、其他狀態拋錯

#### Scenario: 儲存新使用者
- **WHEN** service 完成雜湊與組裝後需存檔
- **THEN** `userRepository.save(user)` 發 `POST {GATEWAY_URL}/internal/users`，body 為 `{ id, email, password(已雜湊) }`
...
成功回應 body 為直接物件（無包裝）；錯誤回應 body MUST 為 `{ error, message }` 結構（`error` 為語意錯誤碼字串）。
...
| 錯誤碼 | HTTP | 使用者訊息 |
|---|---|---|
| EMAIL_ALREADY_EXISTS | 409 | 該電子郵件已被註冊，請更換帳號。 |
| UNKNOWN_USER | 400 | Email或密碼錯誤，請輸入正確的Email或密碼。 |
| EMAIL_OR_PASSWORD_NOTMATCH | 400 | Email或密碼錯誤，請輸入正確的Email或密碼。 |
| （未命中 ERROR_MAP，含 UNKNOWN_SERVER_ERROR） | 500 | 伺服器內部發生錯誤，請稍後再試。（`error` 欄位固定為 `INTERNAL_SERVER_ERROR`） |
| （register 的 validateMiddleware 驗證失敗，無錯誤碼） | 400 | EMAIL或密碼格式不正確（此路徑仍用舊版 `{status:"error",message}` 包裝，見下方輸入驗證需求） |
